            ...
            <div class="row">
                ...
                <div class="instructor-pending">
                    <?php
                        if(isset($_GET['msg'])) {
                            if($_GET['msg'] == 'success') {
                                echo "<div class='panel panel-small panel-success text-center' role='alert'>Deleted Category and Subcategories related to it.</div>";
                            } else {
                                echo "<div class='panel panel-small panel-danger text-center' role='alert'>Error Occured. Cant delete at the moment.</div>";
                            }
                        }
                        $list_categories = $con->query("SELECT id, name FROM course_categories");
                        if(mysqli_num_rows($list_categories) > 0) {
                            ?>
                        <table class="pure-table pure-table-striped" style="margin-bottom: 40px;">
                            <thead>
                                <tr>
                                    <th class="text-center">Cateogory Name</th>
                            ...
                            <tr class="text-center">
                                <td><?= $category['name']; ?></td>
                                <td><a href="delete-category-confirm.php?id=<?= $category['id']; ?>" title="">Delete</a></td>
                            </tr>
                            <?php
                                }
                            ?>
